...
class HookEngine(object):
    """
    store hooks for events and dispatch to these hooks when an event
    is triggered. Event parameters can be modified or added.
    Hook engine also implement the descriptor protocol. If a hook is fetched as
    an instance attribute (iff it was originally a class attribute), a new
    hook will be produced which is bound to the relevant instance. Like
    bound methods, bound hook will pass `self` as the first parameter to
    their event hooks when triggered.
    Bound hook are also persisted; fetching a hook from the same
    instance twice will produce the same bound hook both times. This is
    useful when trying to add new hooks per-instance.
    Normally, an instance can only have one hook bound to it in this way
    because of the way bound hook are stored. To allow an instance to have
    multiple hook bound and stored, one must pass the `id` parameter when
    instantiating a hook. The provided `id` will be used as the store key
    instead of the default.
    If a hook has an `id` of None, binding it will never store the bound
    hook.
    """
    _bound_event_store = weakref.WeakKeyDictionary()
    ...
    def subscribe(self, func=None, needs=None, returns=None,
                  modifies=None, **conditions):
        """Add a hook to a event hook.
        This method can either be used as a decorator for a function or method,
        or standalone on a callable.
        :param func: the func to add the call
        :param needs:
        :param returns:
        :param modifies:
        :param conditions:
         ...
         * `returns` is an iterable of parameters that this hook will modify
           and then pass back to the func.
        ...
        parameters = set(needs) | set(modifies)
        ...
        def deco(_func):
            self.hooks.append(_Hook(_func, needs, parameters, returns, conditions))
            return _func
    ...
    def clone(self):
        ...
        Returns a copy of this hook, with all the same state. Both will be
    ...
    def stored_bind(self, instance):
        ...
        store = self._bound_event_store.setdefault(instance, {})
        p = store.get(self.id)
        if p is None:
    ...
    def from_store(cls, instance, id=_DEFAULT_ID):
        ...
        """
        return cls._bound_event_store[instance][id]

_DEFAULT_AGGREGATE_ID = object()


class HookAggregate(object):
...
class _Hook(object):
    ...
    def matches(self, event):
        if not all(key in event for key in self.needs):
            return False
        if not all(event[key] == self.conditions[key] for key in self.conditions):
            return False
        return True

    def execute(self, hook, event):
        relevant = dict((key, value) for key, value in event.iteritems() if key in self.parameters)
        # Pass the bound instance as the first argument, i.e. self.
        if hook.instance is not None:
            if hook.instance() is None:
                raise InstanceDead()
            args = hook.instance(), hook
        else:
            args = hook,
