...
class RegisterPluginError(Exception):
    pass
...
class TaskRegister(dict):
    ...
    def unregister(self, name):
        ...
        try:
            ...
            raise RegisterPluginError(name)


class PluginManagement(object):

    def __init__(self, name, loader=None,
                 config=None,plugin_cls=None ):
        self.name = name
        self.loader = loader
        self.config = config
    ...
    def plugin(self, *args, **kwargs):
        def inner_load_plugin_cls(**options):
            def _create_plugin(func):
                pass

    # def interface(self,name,group=None,*args,**kwargs):
    #     pass


class PluginManager(object):
    """
    Core Plugin-manager class which manages registration
    of plugin objects and 1:N hook calling.
    You can register new hooks by calling ``add_hookspec(module_or_class)``.
    You can register plugin objects (which contain hooks) by calling
    ``register(plugin)``.  The Plugin-manager is initialized with a
    ...
    def __init__(self, name, impl_prefix=None):
